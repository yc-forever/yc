...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    "get_streams_url": True
		...
		print(resultJo)
		noRsp = requests.get(resultJo['download_url'
],headers=self.header, allow_redirects=False,verify = False)
		realUrl = ''
		if 'Location' in noRsp.headers:
			realUrl = noRsp.headers['Location'
]
    ...
    4
		...
		url = '{
    0
}?do=push_agent&api=python&type=m3u8&share_id={
    1
}&file_id={
    2
}'.format(self.localProxyUrl,shareId,fileId)
		subtitleUrl = self.subtitleContent(id)
		newHeader = {
    "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.54 Safari/537.36",
    "referer": "https://www.aliyundrive.com/",
    ...
    0
		...
		m = re.search('www.aliyundrive.com\\/s\\/([^\\/
]+)(\\/folder\\/([^\\/
]+))?', tid)
		col = m.groups()
		shareId = col[
    ...
    0
		...
		if fileType != 'folder':
			if fileType != 'file' or fileInfo['category'
] != video:
				return ''
			fileId = 'root'
		...
		sortedMap = sorted(hashMap.items(), key=lambda x: x[
    0
])
		arrayList = []
		playList = []
    ...
    "template_id": ""
		...
		videoList = rspJo['video_preview_play_info'
...
"x-oss-expires=(\\d+)&"))
	...
	def proxyM3U8(self,map):
		...
		return [
    200,
    "application/octet-stream", action, content
]
	...
	def getToken(self,shareId,sharePwd):
		...
		self.expiresMap[shareId
] = self.localTime + int(jo['expires_in'
])
		self.shareTokenMap[shareId
] = newShareToken
...
51):
			...
			for jt in ja:
				if jt['type'
					...
					if 'video' in jt['mime_type'
] or 'video' in jt['category'
]:
						repStr = jt['name'
].replace("#",
"_").replace("$",
"_").replace(jt['file_extension'
],'')[
    0: -1
]
						map[repStr
] = shareId + "+" + shareToken + "+" + jt['file_id'
] + "+" + jt['category'
] + "+"
					elif 'others' == jt['category'
] and ('srt' == jt['file_extension'
] or 'ass' == jt['file_extension'
]):
						repStr = jt['name'
].replace("#",
"_").replace("$",
	...
	def login(self):
		...
		if len(self.authorization) == 0 or self.timeoutTick - self.localTime <= 600:
			...
			if rsp.status_code == 200:
				self.authorization = jo['token_type'
] + ' ' + jo['access_token'
]
